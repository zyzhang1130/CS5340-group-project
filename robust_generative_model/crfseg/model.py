...
class CRF(nn.Module):
    ...
    def _smoothing_filter(self, x, spatial_spacings):
        ...
        output : torch.tensor
            Tensor of shape ``(batch_size, n_classes, *spatial)``.
        """
        # result = torch.stack([self._single_smoothing_filter(x[i], spatial_spacings[i]) for i in range(x.shape[0])])
        # return result
        batch_size = x.size(0)
        x = x.flatten(0, 1)
        spatial_spacings = spatial_spacings[0]
        result = self._single_smoothing_filter(x, spatial_spacings)
        result = result.view(batch_size, -1, result.size(1), result.size(2))
        return result
